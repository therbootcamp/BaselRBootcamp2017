...
4. Ok you've got the data from the internet into your R session. But let's save a local copy of the data as a `.csv` file so we'll always have access to it. Using `write_csv()`, save a copy of the data as a `.csv` file called `ACTG175.csv`, also make sure to put it in the data folder you just created.
...
6. Now it's time to get the data back from the local file! To do this, load the `ACTG175.csv` file data back into your R session and again store it as the object `ACTG175`. Now, your code should be able to access your local copy of the data, even when you are not online.

7. The `ACTG175` data is comes from the `speff2trial` package. Install the `speff2trial` package using `install.packages()`, load the package with `library()` and then look at the help menu for the `ACTG175` data by running `?ACTG175`

8. First thing's first, take a look at the first few rows of the data by printing it. It should look like this:
